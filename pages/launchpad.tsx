      ...
      <div className="flex flex-col md:flex-row w-screen backdrop-opacity-10 backdrop-invert bg-[#05325B]/70 h-screen overflow-auto hidden-scrollbar">
        <div className="w-full md:w-80 py-10 px-5 h-[40px] md:h-full bg-[#161525] text-white">
          <LaunchpadNavbar>
            ...
            <NavLink label="Airdrops" icon={<FaParachuteBox className="text-white" />} dropdown onClick={() => setRoute(Routes.PRIVATE_SALES)} />
            <NavLink label="Locks" icon={<FiLock className="text-white" />} dropdown onClick={() => setRoute(Routes.PRIVATE_SALES)} />
            <NavLink
              label="Utitlty &amp; Tools"
            ...
            />
            <NavLink label="Listing Alerts (Beta)" icon={<FiNavigation className="text-white" onClick={() => setRoute(Routes.PRIVATE_SALES)} />} />
            <NavLink label="KYC &amp; Audit" icon={<FiShield className="text-white" />} onClick={() => setRoute(Routes.PRIVATE_SALES)} />
          </LaunchpadNavbar>
        </div>
        <div className="w-screen p-5">
          {route === Routes.PRESALES ? (
            <Presales />
          ) : (
            <div className="flex justify-center items-center w-full my-[100px]">
              <div className="flex flex-col-reverse justify-center items-center gap-6">
